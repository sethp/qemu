                                 ...
                                 struct omap3_boot_s *s)
{
    ...
    switch (s->state) {
        ...
        case imagehdr:
            ...
            address_space_write_rom(&address_space_memory, 0x40014040, 
                                    MEMTXATTRS_UNSPECIFIED, data + 4, 4);
            /* start copying image */
            data += 8;
            data_len -= 8;
            s->state = copy;
...
static int omap3_boot_finish(struct omap3_boot_s *s)
{
    ...
    if (result) {
        ...
        address_space_write_rom(&address_space_memory, 0x40014044, MEMTXATTRS_UNSPECIFIED, x, 12);
    }
    free(s);
    return result;
}
